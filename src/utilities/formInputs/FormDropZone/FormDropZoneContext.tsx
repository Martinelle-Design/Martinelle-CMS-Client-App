...
export const DropZoneProvider = ({
  ...
  const [storedImages, setStoredImages] = useState<MediaLink[]>(
    storedMedia?.images ? storedMedia.images : []
  );
  const [storedVideos, setStoredVideos] = useState<MediaLink[]>(
    storedMedia?.videos ? storedMedia.videos : []
  );
  useEffect(() => {
    setStoredImages(storedMedia?.images ? storedMedia.images : []);
  }, [storedMedia?.images]);
  useEffect(() => {
    setStoredVideos(storedMedia?.videos ? storedMedia.videos : []);
  }, [storedMedia?.videos]);
  const wrapped = {
    newImages,
    newVideos,
